...
| --- | --- | --- |
| `INGEST_TOKEN` | `/opt/metrics/.env` on the box | bearer for `POST /ingest` **and for reads** |
| read-API basic auth | `/etc/nginx/.htpasswd-metrics` | user `grafana`, used by Grafana Cloud |
| read-API bearer map | `/etc/nginx/conf.d/metrics-token.conf` | **generated by `deploy.sh`** from `.env`; never edit, never commit |
...
**The read API takes either credential.** Grafana Cloud uses basic auth; clients that already
hold `INGEST_TOKEN` send it as a bearer instead, so an app needs one secret rather than two.
nginx can't validate a bearer itself, so `deploy.sh` writes a `map` that resolves a matching
`Authorization` header to the realm `"off"` — which disables `auth_basic` for that request —
and everything else to the realm name. Rotating the token means editing `.env` and
redeploying; there is nothing to keep in sync.

`/api/v1/export` is on the read allowlist so a client can pull back **raw stored samples**.
`query_range` is not a substitute: it evaluates on a fixed grid, and
`-search.maxStalenessInterval=720h` carries values forward for 30 days, so it reports points
that were never recorded. The allowlist regex is anchored, so `/export/native` and
`/export/csv` stay 404.
